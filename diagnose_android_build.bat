...
setlocal
echo ==========================================
echo   ADVAYA FM ELITE - DIAGNOSTIC & BUILD
echo ==========================================
...
:: 4. CLEAN AND CAPTURE
echo [LOG] Running Clean...
call flutter clean > ../clean_log.txt 2>&1

:: 5. PUB GET AND CAPTURE
echo [LOG] Running Pub Get...
call flutter pub get > ../pub_log.txt 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] Pub Get Failed. Checked pub_log.txt
    copy ../pub_log.txt ../ALL_LOGS.txt
    pause
    exit /b 1
)

:: 6. BUILD AND CAPTURE
echo [LOG] Building AppBundle...
call flutter build appbundle --release --verbose > ../build_log.txt 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] Build Failed. Checked build_log.txt
    copy ../build_log.txt ../ALL_LOGS.txt
    pause
    exit /b 1
)
